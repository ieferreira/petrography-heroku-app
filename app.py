...
if file:
    ...
    color_model = st.sidebar.selectbox(
        "Color Model", ("RGB", "GRAY", "HSL", "HSV", "YBR"))
    ...
    if gauss:
        ksize = st.sidebar.number_input(
            "Size of kernel (odd)", 3, 15, step=2, key='ksize')
        try:
    ...
    # 1784 x 872
    if not gauss:
        mshift = st.sidebar.radio(
            "Mean Shift Filter (Gaussian en False)", (False, True), key='meanshift')
        if mshift:
            sp = st.sidebar.number_input(
                "sp: radio ventana", 1, 500, 20, step=10, key='sp')
            sr = st.sidebar.number_input(
                "sr: radio ventana de color", 1, 500, 50, step=10, key='sr')
            img = mean_shift(img, sp, sr)
            img_mshift = img.copy()

    st.sidebar.markdown("### Edge detection")
    st.sidebar.write("Use HED only once per session, it crashes when used again")
    bordes = st.sidebar.radio(
        "", ("None", "Canny", "Sobel", "Prewitt", "HED"), key="border")  # , "HED"#
    ...
    if bordes == "None":
        ...
        if bordes == "Canny":
            lw = st.sidebar.number_input(
                "Lower threshold", 1, 500, 100, step=10, key='l')
            hg = st.sidebar.number_input(
                "Upper threshold (around twice or thrice lower)", 2, 500, 200, step=10, key='s')
            ...
            if st.sidebar.radio("Binarize ☯", (False, True), key="bin"):
                low = st.sidebar.number_input(
                    "Lower threshold (binarization)", 0, 254, 100, step=10, key='low')
                high = st.sidebar.number_input(
                    "Upper threshold (binarization)", 1, 255, 255, step=10, key='high')
        ...
        if st.sidebar.radio("Watershed", (False, True), key="watershed"):
            ...
            if st.sidebar.checkbox("Show statistical data: ", False):
                st.subheader(
                    "Statistical data found (units are pixels, pixels$^2$ or degrees)")
                df = pd.read_csv("medidas.csv")
                df = df.drop(columns=[
                             "Unnamed: 0", "MinIntensity", "MeanIntensity", "MaxIntensity", "Perimeter.1"])
            ...
            try:
                if mshift == True:
                    if st.sidebar.radio("SLIC (Clusters)", (False, True), key="slic"):
                        ...
                        st.write(
                            f"Usando los granos encontrados en Watershed, se encuentran {len(np.unique(segments))} usando SLIC")
        ...
        if bordes == "Canny":
            if st.sidebar.radio("Find Contours", (False, True), key="fcontours"):
                treshold = st.sidebar.number_input(
                    "Umbral desde el cual contar contornos", 1, 300, 50, step=10, key='th')
...
st.markdown(
    "Programmed by Iván Ferreira, UnalGeo-Bogotá (2020). [Github! 🎯](https://github.com/ieferreira)")
